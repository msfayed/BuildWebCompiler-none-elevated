...
namespace WebCompiler
{
	internal class BabelCompiler : ICompiler
	{
		...
		private void RunCompilerProcess(Config config, FileInfo info)
		{
			...
			processStartInfo.Arguments = "/c \"\"" + Path.Combine(_path, "node_modules\\.bin\\babel.cmd") + "\" " + text + " \"" + info.FullName + "\"\"";
		...
		{
			string text = "--presets react --out-file \"\"";
			if (BaseOptions<BabelOptions>.FromConfig(config).SourceMap || config.SourceMap)
			{
				text += " --source-maps inline";
			}
			return text;
		}
	}
